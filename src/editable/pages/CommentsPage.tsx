...
export default function CommentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return comments.filter((item) =>
      [item.name, item.email, item.comment, item.articleTitle, item.articleSlug].filter(Boolean).some((value) => String(value).toLowerCase().includes(term))
    )
  }, [comments, query])
  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[1440px] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2.2rem] bg-white p-6 shadow-[0_24px_80px_rgba(40,9,5,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--slot4-soft-muted-text)]">Review comments saved in this browser from article pages.</p>
            </div>
            <button type="button" className="rounded-full border border-[color:rgba(40,9,5,0.12)] px-4 py-2 text-sm font-black uppercase tracking-[0.12em]" onClick={refreshComments}>
              Refresh comments
            </button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-12 w-full rounded-full border border-[color:rgba(40,9,5,0.12)] bg-[#fff8f4] pl-10 pr-4 text-sm font-bold outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[1.8rem] bg-white p-5 shadow-[0_18px_48px_rgba(40,9,5,0.06)]">
          ...
          <section className="mt-8 rounded-[2rem] bg-white p-8 text-center shadow-[0_18px_48px_rgba(40,9,5,0.06)]">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[1.8rem] bg-white p-4 text-sm text-[var(--slot4-soft-muted-text)] shadow-[0_18px_48px_rgba(40,9,5,0.06)]">
            <span>
              Page {currentPage} of {totalPages}
            </span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[color:rgba(40,9,5,0.12)] px-4 py-2 font-black uppercase tracking-[0.12em] disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>
                Previous
              </button>
              <button type="button" className="rounded-full border border-[color:rgba(40,9,5,0.12)] px-4 py-2 font-black uppercase tracking-[0.12em] disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>
                Next
              </button>
            </div>
          </div>
