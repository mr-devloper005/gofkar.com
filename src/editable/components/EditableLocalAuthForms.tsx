...
const inputClass =
  'h-12 rounded-full border border-[color:rgba(40,9,5,0.12)] bg-white px-4 text-base font-bold text-black outline-none transition placeholder:text-black/35 focus:border-[var(--slot4-accent)]'
const buttonClass =
  'inline-flex h-12 items-center justify-center rounded-full bg-[var(--slot4-dark-bg)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--slot4-dark-text)] shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.2rem] px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
